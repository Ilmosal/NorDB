...
def insertcha(repo, channel_file):
    ...
    if fnmatch.fnmatch(channel_file, "*.sitechan"):
        station2sql.readChannels(open(channel_file, 'r'))
    else:
...
def insertsta(repo, station_file, network):
    ...
    if fnmatch.fnmatch(station_file, "*.site"):
        station2sql.readStations(open(station_file, 'r'), network)
    else:
...
def getsta(repo, output, o_format, network):
    """
    This command fetches the stations that match the criteria given by user and parses them to output.
    """
    if o_format == "site":
...
@click.option('--event-id-file', '-idf', type=click.Path(exists=True, readable=True), help="link to a id file which you havve created with search commands --output flag")
